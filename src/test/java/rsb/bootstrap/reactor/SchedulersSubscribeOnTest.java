...
public class SchedulersSubscribeOnTest {
    ...
    public void subscribeOn() {
        ...
        var integer = Mono.just(1).subscribeOn(scheduler).doFinally(signalType -> {
            map.forEach((s, atomicInteger) -> log.info("{} = {}", s, atomicInteger));
        });

        StepVerifier.create(integer).expectNextCount(1).verifyComplete();
        assertEquals(1, map.get(rsbThreadName).get());
    }
}
